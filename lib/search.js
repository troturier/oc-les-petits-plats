...
/** Query string key holding the main search term. */
export const QUERY_PARAM = "q";

/**
 * Reads the selected tags out of the URL search params. Values absent from the
 * dataset are dropped, so a hand-written URL cannot inject a phantom tag that
 ...
 */
export function readTagsFromParams(params, recipes) {
  /** @type {Record<FilterCategory, Set<string>>} */
  const known = {
    ingredients: new Set(
      recipes.flatMap((recipe) =>
        recipe.ingredients.map((item) => normalize(item.ingredient)),
      ),
    ),
    appliances: new Set(recipes.map((recipe) => normalize(recipe.appliance))),
    ustensils: new Set(
      recipes.flatMap((recipe) => recipe.ustensils.map(normalize)),
    ),
  };
  ...
  for (const category of FILTER_CATEGORIES) {
    ...
    const values = raw === undefined ? [] : [raw].flat();

    tags[category] = [...new Set(values.map(normalize))].filter((value) =>
      known[category].has(value),
    );
  }
...
/**
 * Serializes the current criteria back into a query string. The main search is
 * only kept once it is long enough to actually filter.
 *
 * @param {string} query
 * @param {SelectedTags} tags
 * @returns {string}
 */
export function buildSearchParams(query, tags) {
